...
/**
 * Repositorio para ServicioSkill.
 */
@Repository
public interface ServicioSkillDAO extends JpaRepository<ServicioSkill, ServicioSkillId> {
    ...
    /**
     * Devuelve los ids de skills requeridas por una lista de servicios.
     */
    @Query("SELECT DISTINCT ss.skill.id FROM ServicioSkill ss WHERE ss.servicio.id IN :servicioIds")
    List<Integer> obtenerSkillIdsRequeridas(@Param("servicioIds") List<Integer> servicioIds);
}